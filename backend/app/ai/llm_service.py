"""LLM Service for tender requirement extraction and intelligence analysis using Groq Multi-Key Router."""
...
async def analyze_tender_with_llm(text: str) -> TenderAnalysisResult:
    """Analyzes raw tender document text using Groq LLM with round-robin fallback and returns a validated TenderAnalysisResult.
    ...
    try:
        ...
        logger.error("Groq AI router error during tender analysis: %s", err)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Groq API tender analysis error: {str(err)}",
        ) from err
